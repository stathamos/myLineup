...
def get_queries():
    data = [['Traditional power forward', 'select CASE WHEN T.Type = "2 - 1" THEN "Traditional power forward" ELSE "Other players" END as Type,'
         ...
         ['Traditional center', 'select CASE WHEN T.Type = "2 - 0" THEN "Traditional center" ELSE "Other players" END as Type,'
         ...
         ['Back-up center', 'select CASE WHEN T.Type = "2 - 2" THEN "Back-up center" ELSE "Other players" END as Type,'
                                   '"PlayersPlaytypes_PRRollman_POSS" from "PlayersPlaytypes_PRRollman" P LEFT JOIN '
                                   '"Players_with_type" T on P."PlayersPlaytypes_PRRollman_PLAYER_ID" = '
                                   'T.PlayersBios_PLAYER_ID WHERE PlayersPlaytypes_PRRollman_SeasonYear = "2020-21" AND ' 
                                   'PlayersPlaytypes_PRRollman_SeasonType = "Regular Season"', 'Number of screens sets'],
         ['Top guards', 'select CASE WHEN T.Type = "4 - 0" THEN "Top guards" ELSE "Other players" END as Type,'
                                   ...
                                   'PlayersShot7DribbleRange_SeasonType = "Regular Season"', 'Number of shots taken after'
                                    '7 or more dribbles'],
         ['Traditional point guard', 'select CASE WHEN T.Type = "4 - 1" THEN "Traditional point guard" ELSE "Other players" END as Type,'
         ...
         ['Complete superstar', 'select CASE WHEN T.Type = "4 - 2" THEN "Complete superstar" ELSE "Other players" END as Type,'
         ...
         ['Elite modern bigs', 'select CASE WHEN T.Type = "0 - 1" THEN "Elite modern bigs" ELSE "Other players" END as Type,'
         ...
         ['Modern bigs', 'select CASE WHEN T.Type = "0 - 2" THEN "Modern bigs" ELSE "Other players" END as Type,'
                                   ...
                                   'PlayersGeneralStats_SeasonType = "Regular Season"',
          'Number of points + number of rebounds per game'],
         ['Elite traditional centers', 'select CASE WHEN T.Type = "0 - 0" THEN "Elite traditional center" ELSE "Other players" END as Type,'
         ...
         ['Elite catch and shooters', 'select CASE WHEN T.Type = "6 - 2" THEN "Elite shooter" ELSE "Other players" END as Type,'
         ...
         ['Low playing time forward', 'select CASE WHEN T.Type = "5 - 2" THEN "Low playing time forward" ELSE "Other players" END as Type,'
         ...
         ['Low playing time center', 'select CASE WHEN T.Type = "5 - 1" THEN "Low playing time center" ELSE "Other players" END as Type,'
                                   'PlayersGeneralStats_Min from "PlayersGeneralStats" P LEFT JOIN '
                                   '"Players_with_type" T on P."PlayersGeneralStats_PLAYER_ID" = '
                                   'T.PlayersBios_PLAYER_ID WHERE PlayersGeneralStats_Season = "2020-21" AND '
                                   'PlayersGeneralStats_SeasonType = "Regular Season"', 'Number of minutes played per game'],
         ['Back up space creator', 'select CASE WHEN T.Type = "1 - 1" THEN "Back up space creator" ELSE "Other players" END as Type,'
                                   'PlayersGeneralStats_Min from "PlayersGeneralStats" P LEFT JOIN '
                                   '"Players_with_type" T on P."PlayersGeneralStats_PLAYER_ID" = '
                                   'T.PlayersBios_PLAYER_ID WHERE PlayersGeneralStats_Season = "2020-21" AND '
                                   'PlayersGeneralStats_SeasonType = "Regular Season"', 'Number of minutes played per game'],
         ['Back up center', 'select CASE WHEN T.Type = "1 - 2" THEN "Back up center" ELSE "Other players" END as Type,'
                                   'PlayersGeneralStats_Min from "PlayersGeneralStats" P LEFT JOIN '
                                   '"Players_with_type" T on P."PlayersGeneralStats_PLAYER_ID" = '
                                   'T.PlayersBios_PLAYER_ID WHERE PlayersGeneralStats_Season = "2020-21" AND '
                                   'PlayersGeneralStats_SeasonType = "Regular Season"', 'Number of minutes played per game'],
         ['Back up point guard', 'select CASE WHEN T.Type = "1 - 0" THEN "Back up point guard" ELSE "Other players" END as Type,'
         ...
         ['Decent point guard', 'select CASE WHEN T.Type = "3 - 0" THEN "Decent point guard" ELSE "Other players" END as Type,'
                                   '"PlayersGeneralStats_AST" from "PlayersGeneralStats" P LEFT JOIN '
                                   '"Players_with_type" T on P."PlayersGeneralStats_PLAYER_ID" = '
                                   'T.PlayersBios_PLAYER_ID WHERE PlayersGeneralStats_Season = "2020-21" AND '
