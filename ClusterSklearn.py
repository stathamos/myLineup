...
def get_dist_from_centroid():
    ...
    for i in range(len(PCA_components_with_distances)):
        distances.append(np.sqrt((PCA_components_with_distances['PCA1'][i]-PCA_components_with_distances['cx'][i])**2
    ...
    PCA_components_with_avg_distances = pd.merge(PCA_components_with_distances, avg_dist_by_cluster_df, how='inner', on='Cluster')
    PCA_components_with_avg_distances.rename(columns={'Name_x' : 'Name', 'Distances_x' : 'Distance from centroid', 'Distances_y' : 'Average distance from centroid'}, inplace=True)
    PCA_components_with_avg_distances['Diff distance'] = abs(PCA_components_with_avg_distances['Distance from centroid'] - PCA_components_with_avg_distances['Average distance from centroid'])
    PCA_components_with_avg_distances['Ratio'] = abs(PCA_components_with_avg_distances['Distance from centroid'] / PCA_components_with_avg_distances['Average distance from centroid'])
    return PCA_components_with_avg_distances


def plot_dist_from_centroid():
    fig2 = px.scatter(PCA_components_with_avg_distances, x='Diff distance', y='Cluster', color='Ratio', hover_name='Name')
    return fig2
